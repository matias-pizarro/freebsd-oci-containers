...
postgres_userdir="/var/db/postgres"
...
docker_create_db_directories() {
	...
	mkdir -p "$UNIX_SOCKET_DIRECTORIES" || :
	chmod 03775 "$UNIX_SOCKET_DIRECTORIES" || :
	...
	if [ "$user" = '0' ]; then
		...
		find "$UNIX_SOCKET_DIRECTORIES" \! -user postgres -exec chown postgres '{}' +
	fi
}
...
docker_process_sql() {
	if [ -S "${UNIX_SOCKET_DIRECTORIES}/.s.PGSQL.5432" ]; then
		CUSTOM_UNIX_SOCKET_DIRECTORY="${UNIX_SOCKET_DIRECTORIES}"
	else
		CUSTOM_UNIX_SOCKET_DIRECTORY=""
	fi
	local query_runner=( psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --no-password --no-psqlrc ${CUSTOM_UNIX_SOCKET_DIRECTORY:+-h ${CUSTOM_UNIX_SOCKET_DIRECTORY}} )
...
docker_setup_env() {
	...
	elif [ "$PGDATA" = "${postgres_userdir}/$PG_MAJOR/docker" ]; then
		# https://github.com/docker-library/postgres/pull/1259
		for d in ${postgres_userdir} ${postgres_userdir}/data ${postgres_userdir}/*/docker; do
...
_pg_want_help() {
	...
	for arg; do
		case "$arg" in
			...
			# postgres: could not access the server configuration file "${postgres_userdir}/data/postgresql.conf": No such file or directory
