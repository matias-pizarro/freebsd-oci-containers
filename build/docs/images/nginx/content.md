...
podman run --name some-nginx -d some-content-nginx
```
...
```console
podman run --rm --entrypoint=cat nginx /etc/nginx/nginx.conf > /host/path/nginx.conf
```
...
Then build the image with `podman build -t custom-nginx .` and run it as follows:

```console
podman run --name my-custom-nginx-container -d custom-nginx
```
...
```console
podman run -d -e NGINX_ENTRYPOINT_QUIET_LOGS=1 nginx
```
...
Alternatively, check out the official [Docker NGINX unprivileged image](https://hub.podman.com/r/nginxinc/nginx-unprivileged).
